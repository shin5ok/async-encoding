...
requesting:
	...
	gcloud run deploy requesting --region=$(REGION) --set-env-vars=BASE_URL=$(BASE_URL),PROJECT_ID=$(GOOGLE_CLOUD_PROJECT) --image=$(BASE_REPO)/requesting --allow-unauthenticated

.PHONY: delivering
delivering:
	docker build -t $(BASE_REPO)/delivering -f ./delivering/Dockerfile ./delivering
	docker push $(BASE_REPO)/delivering
	gcloud run deploy delivering --region=$(REGION) --set-env-vars=TOPIC=$(TOPIC) --image=$(BASE_REPO)/delivering --allow-unauthenticated
